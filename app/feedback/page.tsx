...
export default function FeedbackPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setStatus('success')
      setForm({ feedback_type: '', program: '', message: '', name: '', email: '' })
    }
  }
              ...
              <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
                <div>
                  ...
                  >
                    ...
                    {FEEDBACK_TYPES.map(opt => <option key={opt}>{opt}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  >
                    ...
                    {PROGRAMS.map(opt => <option key={opt}>{opt}</option>)}
                  </select>
                </div>
                <div>
                  <label className="form-label">Your Feedback</label>
                  <textarea
                    placeholder="Share what you would like us to know..."
                    rows={6}
                ...
                <div style={{ background: 'white', borderRadius: '2px', padding: '16px', border: '1px solid var(--border)' }}>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <input
                        type="email"
                        placeholder="Optional — required only if you want a response"
                        className="form-input"
                        value={form.email}
